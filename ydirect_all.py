...
def get_all_direct():
    ...
    def get_direct_cabinet(token, cabinet):
        ...
        date_from = ydirect_constants.all_time_date_from
        # Получение тел запросов для 2 отчетов
        data = ydirect_constants.get_all_data()
        data2 = ydirect_constants.get_all_data2()
    ...
    df_campaign1.to_sql(
        name="direct_campaign_performance",
        schema="ods",
        con=engine,
        if_exists="append",
        index=False
    ...
    df_criteria1.to_sql(
        name="direct_criteria_performance",
        schema="ods",
        con=engine,
        if_exists="append",
        index=False
    ...
    direct_from_db = pd.read_sql_table('direct_campaign_performance', conn_string, schema='ods')
    ...
    CAMPAIGN_PERFORMANCE_REPORT2.to_sql(
        name="direct_campaign_performance",
        schema="ods_plus",
        con=engine,
        if_exists="replace",  # перезапись
        index=False
    )
    # Для отладки
    print('Запись 1 отчета окончена для ods_plus')
    # Получаем все direct_criteria_performance из ods
    criteria_from_db = pd.read_sql_table('direct_criteria_performance', conn_string, schema='ods')
    # Список для уникальных записей, для ускорения записи по 1 кортежу
    ...
    CRITERIA_PERFORMANCE_REPORT2.to_sql(
        name="direct_criteria_performance",
        schema="ods_plus",
        con=engine,
        if_exists="replace",  # перезапись
